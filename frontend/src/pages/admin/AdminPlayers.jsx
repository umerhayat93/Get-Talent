            ...
            <div key={p.id} style={{ background:'#1a2a3a', border:'1px solid rgba(0,0,0,0.07)', borderRadius:16, overflow:'hidden' }}>
              ...
              {expandedId===p.id && (
                <div className="fade-in" style={{ padding:'0 16px 16px', borderTop:'1px solid rgba(0,0,0,0.06)' }}>
                  ...
                  {p.paymentReceipt && (
                    <a href={p.paymentReceipt?.startsWith('data:') ? p.paymentReceipt : `${(import.meta.env.VITE_API_URL || 'https://get-talent-api.onrender.com/api').replace('/api','')}/uploads/${p.paymentReceipt}`} target="_blank" rel="noreferrer" style={{ display:'block', marginBottom:10, padding:'8px 12px', background:'rgba(64,169,255,0.1)', border:'1px solid rgba(64,169,255,0.25)', borderRadius:10, fontSize:13, color:'#40a9ff', fontWeight:600 }}>

...
export default function RegisterCaptainPage() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', teamName: '', email: '', password: '', confirm: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/register/captain', {
        name: form.name.trim(),
  ...
  if (step === 'success') return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: '#f0f2f5' }}>
      <div style={{ textAlign: 'center', maxWidth: 340, width: '100%' }}>
        <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'rgba(245,200,66,0.12)', border: '2px solid rgba(245,200,66,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36, margin: '0 auto 20px' }}>👑</div>
        ...
        <button onClick={() => setShowPayment(true)} style={{ width: '100%', padding: '12px', borderRadius: 13, background: 'transparent', border: '1px solid rgba(245,200,66,0.3)', color: '#1877f2', fontSize: 14, cursor: 'pointer', fontWeight: 600, marginBottom: 8 }}>
  ...
  const fields = [
    ...
    { key: 'teamName', label: 'Team Name',        type: 'text',     ph: 'e.g. Pattan Lions'    },
    ...
    <div style={{ minHeight: '100vh', padding: '28px 20px 48px', background: '#f0f2f5', maxWidth: 440, margin: '0 auto' }}>
      ...
      <div style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 20, padding: '22px 18px' }}>
        <form onSubmit={handleSubmit} noValidate>
          ...
          <button type="submit" disabled={loading} style={{ width: '100%', padding: '14px', borderRadius: 14, background: loading ? '#e4e6ea' : 'linear-gradient(135deg,#f5c842,#e6a800)', border: 'none', color: loading ? '#9090a8' : '#ffffff', fontSize: 16, fontWeight: 800, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginTop: 8 }}>
            {loading ? <><div style={{ width: 18, height: 18, border: '2.5px solid rgba(255,255,255,0.2)', borderTopColor: '#888', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />Registering...</> : 'Register as Captain'}
          </button>

...
function WinnerModal({ winner, player, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,0.9)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background: '#ffffff', border: '1px solid #e4e6ea', borderRadius: 24, padding: '32px 28px', maxWidth: 360, width: '100%', textAlign: 'center' }} className="scale-in">
...
export default function BiddingPage() {
  ...
  const timerHours = Math.floor(timeLeft / 3600).toString().padStart(2, '0');
  const timerMins  = Math.floor((timeLeft % 3600) / 60).toString().padStart(2, '0');
  const timerSecs  = (timeLeft % 60).toString().padStart(2, '0');
  const timerDisplay = timeLeft >= 3600 ? `${timerHours}h ${timerMins}m` : timeLeft >= 60 ? `${timerMins}:${timerSecs}` : `${timerSecs}s`;
  const timerColor = timeLeft <= 60 ? '#ff4444' : timeLeft <= 300 ? '#ff9500' : '#00e676';
  const topBid     = bids[0];
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {tournaments.map(t => (
            <div key={t.id} onClick={() => { setSelectedTournament(t); setView('room'); }}
              style={{ background: '#ffffff', border: '1px solid #e4e6ea', borderRadius: 18, padding: '18px 16px', cursor: 'pointer', transition: 'transform 0.15s' }}
        ...
        {activeSessions.length > 0 && (
          <div style={{ margin: '0 16px 16px' }}>
            ...
            {activeSessions.map(s => (
              <div key={s.id} onClick={() => joinSession(s)} style={{ background: '#ffffff', border: '2px solid #42b72a', borderRadius: 14, padding: '12px 14px', cursor: 'pointer', display: 'flex', gap: 12, alignItems: 'center', marginBottom: 8, boxShadow: '0 2px 12px rgba(66,183,42,0.15)' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <span style={{ fontWeight: 700, fontSize: 14, color: '#050505' }}>{s.player?.name}</span>
                    <VerifiedBadge size={13} />
                  </div>
                  <div style={{ fontSize: 12, color: '#65676b' }}>{s.player?.skill} · {s.player?.category} · Min Rs.{(s.player?.minimumBid || 0).toLocaleString()}</div>
                </div>
                <div style={{ background: '#42b72a', color: '#fff', borderRadius: 9999, padding: '6px 14px', fontSize: 12, fontWeight: 800, border: 'none' }}>JOIN →</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ margin: '10px 16px', background: isDiamond ? 'linear-gradient(135deg,#0a1e35,#0d2440)' : 'linear-gradient(135deg,#0f2d1e,#1a3a28)', border: `1px solid ${isDiamond ? 'rgba(64,169,255,0.25)' : 'rgba(0,230,118,0.25)'}`, borderRadius: 20, padding: '16px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(0,0,0,0.3)', borderRadius: 9999, padding: '4px 12px' }}>
            ...
            <div style={{ textAlign: 'right' }}><span style={{ fontFamily: "'Bebas Neue',cursive", fontSize: 22, color: timerColor, letterSpacing: 1 }}>{timeLeft <= 0 ? 'ENDED' : timerDisplay}</span><div style={{ fontSize: 9, color: 'rgba(255,255,255,0.5)', fontWeight: 700, letterSpacing: '0.8px' }}>TIME LEFT</div></div>
          </div>
        </div>
        {topBid ? (
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <CheckMark size={16} />
              <span style={{ fontSize: 13, fontWeight: 700, color: '#ffffff' }}>{topBid.captain?.teamName}</span>
            </div>
          </>
        ) : (
